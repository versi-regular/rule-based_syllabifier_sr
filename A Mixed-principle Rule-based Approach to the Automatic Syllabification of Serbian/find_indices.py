...
def find_vowels(word):
	...
	vowels_positions = {position.start() for position in re.finditer("[АЕИОУаеиоу]", word)}
...
def find_syllabic_consonants(word):
	...
	syllabic_r = {position.start() for position in re.finditer("((?<=^)|(?<=[БВГДЂЖЗЈКЛЉМНЊПСТЋФХЦЧЏШбвгдђжзјклљмнњпстћфхцчџш]))[Рр]([БВГДЂЖЗКЛЉМНЊПСТЋФХЦЧЏШбвгдђжзклљмнњпстћфхцчџш]|[Jј](!?=е))", word)}
	# l in lower sonority contexts (exclude words with double l)
	syllabic_l = {position.start() for position in re.finditer("((?<=^)|(?<=[БВГДЂЖЗКЉМНЊПСТЋФХЦЧЏШбвгдђжзкљмнњпстћфхцчџш]))[Лл]([БВГДЂЖЗКЉМНЊПСТЋФХЦЧЏШбвгдђжзкљмнњпстћфхцчџш]|(?=$))", word)}
	# n in lower sonority contexts (exclude words with double n)
	syllabic_n = {position.start() for position in re.finditer("((?<=^)|(?<=[БВГДЂЖЗКЉМЊПСТЋФХЦЧЏШбвгдђжзкљмњпстћфхцчџш]))[Нн]([БВГДЂЖЗКЉМЊПСТЋФХЦЧЏШбвгдђжзкљмњпстћфхцчџш]|(?=$))", word)}
